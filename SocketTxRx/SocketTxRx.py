...
def getArguments():
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="IP Address to receive packets",
    )
    parser.add_argument(
        "--tx-ip", dest="TxIp", default="", type=str, help="IP Address to send packets"
    )
    parser.add_argument(
        "--rx-port", dest="RxPort", default="", type=int, help="Port to receive packets"
    )
    parser.add_argument(
        "--tx-port", dest="TxPort", default="", type=int, help="Port to send packets"
    )
    parser.add_argument(
        "--script-timeout",
        dest="timeout",
        default="5",
        type=int,
        help="Script elapse time in seconds before it will exit",
    )


def openMulticastUdpTx(TxIp, TxPort):
    # init the socket
    TransmitSock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # Internet  # UDP
...
def openMulticastUdpRx(RxIp, RxPort):
    ReceiveSock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # Internet  # UDP
    try:
        ReceiveSock.setsockopt(socket.SOL_SOCKET, socket.SOCK_DGRAM)
    ...
    ReceiveSock.setsockopt(
        socket.SOL_IP, socket.IP_MULTICAST_IF, socket.inet_aton(host)
    )
    ReceiveSock.setsockopt(
        socket.SOL_IP,
        socket.IP_ADD_MEMBERSHIP,
        socket.inet_aton(RxIP) + socket.inet_aton(host),
    )
    return ReceiveSock
